...
| Tag freshness | SQLite `state_metadata.tag_refresh_v1` and `.eidetic/configurations/` | Recipe/input/output fingerprints and immutable selected vocabulary; published with generated tags. |
...
The [release and refresh contract](UPDATES-AND-REFRESH.md) is separate from the
database schema version. Package SemVer identifies software; bundled file hashes
verify the installation. Extractor versions identify reusable measurements.
Tag fingerprints detect role/rule/input changes independently of unrelated search
code. Existing schema 5 stores these records without a migration. Human decisions
and frozen plans are not regenerated by a derived-data refresh.
